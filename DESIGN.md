...
- **히어로(`page-hero`):** `primary-container` 배경, 패딩 `36px 20px 30px`, 가운데 정렬. 제목(우리 동네 목소리함) 26px/700, 설명 14px/400(최대 폭 420px, 불투명도 .85), `button-primary` 1개("의견 남기기"). **홈 화면에만 있다** — 로그인·마이페이지·관리자 화면에는 히어로를 넣지 않는다.
